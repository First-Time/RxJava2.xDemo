...
//        odds.mergeWith(evens)
        ...
        //==========================P161(zip：通过一个函数将多个Observable的发射物结合到一起，基于这个函数的结果为每个结合体发射单个数据项。)=================================
        //zip操作符返回一个Observable，它使用这个函数按顺序结合两个或多个Observable发射的数据项，返回发射这个函数返回的结果。它按照严格的顺序应用这个函数，只发射与发射数据项最少的那个Observable一样多的数据。
        /*Observable<Integer> odds = Observable.just(1, 3, 5, 7, 9);
        Observable<Integer> evens = Observable.just(2, 4, 6);
        ...
        /*Observable<Integer> odds = Observable.just(1, 3, 5,7,9);
        Observable<Integer> evens = Observable.just(20, 40, 60);
        ...
        //join操作符的效果类似排列组合，把第一个数据源A作为基座窗口，它根据自己的节奏不断发射数据元素；第二个数据源B，每发射一个数据，我们都把它和第一个数据源A中已经发射的数据进行一对一匹配。
        ...
        odds.join(evens, new Function<Integer, ObservableSource<String>>() {
            ...
            @Override
            public ObservableSource<String> apply(Integer integer) throws Exception {
                return Observable.just(String.valueOf(integer)).delay(10, TimeUnit.MILLISECONDS);
            }
...
//        Observable<Long> odds = Observable.interval(1, TimeUnit.SECONDS);
        /*Observable<Long> odds = Observable.just(1L,3L,5L);
